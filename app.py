...
with tab1:
    ...
    binary_labels = [
        'HIV_Ag', 'Serum_NTT', 'Serum_TPPA', 'Serum_IgG',
        'CSF_IgG', 'CSF_TRUST/RPR', 'CSF_TPPA', 'CSF_VDRL', 'Neuro_Sympt'
    ]
    ...
    if st.button("Predict"):
        ...
        model_path = f"models/{guidance_folder}_{model_name}.pkl"

        try:
            model = joblib.load(model_path)
            prob = model.predict_proba(input_df)[0][1]
            st.write(f'Neurosyphilis for {selected_guidance} using {selected_model.lower()}')
...
with tab2:
    ...
    model1 = model_options[selected_model].lower()
    fig_filename = f"{guidance1} {model1} auc.tif"
